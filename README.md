...
* **Deterministic CPU Flatline (17–20% Processor Utilization Plateau):** Throughout the entire 75-minute execution loop, the operating system's CPU utilization graph remained completely frozen, forming a strict flat plateau at **17–20% of total processor capacity**. On your 4-core / 8-thread Intel i7 CPU, this exact metric mathematically represents a **100% saturation of a single thread** running under an absolute compute lock at ~5,901 MHz, with the remaining delta assigned to baseline Windows/MINGW64 OS overhead. Concurrently, the core script maintained a negligible, constant dynamic memory footprint of exactly **10,168 BYTES** (with a peak process footprint of **20.52 KB**). Despite evaluating 15,231 massive 1,587-digit integers, there was absolute zero memory drift or heap expansion, proving the total absence of memory leaks inside the YUCT register-bound architecture.
